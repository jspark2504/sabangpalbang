/* 기존 파일 수정 처리 */
function updateStatus(btn, fileNo) {
	//alert('fileNo : ' + fileNo);
//	let files = $('#removedFiles').val()
//	files += ("," + fileNo);
//	$('#removedFiles').val() = files;
//	$(btn).closest("div").remove();		// 이미지 삭제
	roomNo = $('#roomNo').val();

	$.ajax({
		url: "/findMyRoom/room/update/file",
		type: "get",
		data: {
				roomNo : roomNo,
				fileNo : fileNo
		...
		success: function(data) {
			...
			if(data == '1'){
				...
				alert('이미지 삭제에 실패하였습니다. 잠시 후 다시 시도해주세요.')
			}
		},
